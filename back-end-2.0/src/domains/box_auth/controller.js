async function box_auth_controller(box_auth_flag, password){
    ...
    if (!box_auth_flag && password == process.env.DB_PASSWORD) {
        ...
        return_obj.data.redirect_url = `https://account.box.com/api/oauth2/authorize?client_id=${process.env.CLIENT_ID}&response_type=code&redirect_uri=https://node.asharalvany.com/redirect`;
